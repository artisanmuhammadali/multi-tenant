            ...
            <div class="card-body px-0 pb-2">
              <div class="table-responsive p-0">
                <table class="table align-items-center mb-0">
                  ...
                  <tbody>
                    <tr v-if="tenantStore.data ? tenantStore.data.length : false"
                        v-for="tenant in tenantStore.data">
                      <td>
                      ...
                      <td class="align-middle text-sm">
                        <span class="badge badge-sm " :class="tenant.is_active ? 'bg-gradient-success' : 'bg-gradient-danger'">{{ tenant.is_active ? 'Active' : 'In-Active' }}</span>
                      </td>
                      <td class="align-middle">
                        <span class="text-secondary text-xs font-weight-bold">{{ formatDate(tenant.created_at , 'MMMM DD, YYYY') }}</span>
                      </td>
                      <td class="align-middle">
                ...
                <ListPagination
                  :current-page="tenantStore.currentPage"
                  :total-pages="tenantStore.totalPages"
                  :total-items="tenantStore.dataCount"
                  :method="tenantStore.pagination"
